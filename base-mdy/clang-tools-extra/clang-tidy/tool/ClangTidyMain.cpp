...
static std::string GetKeyGenFilePath(const char *argv0)
{
	std::string res;
	std::string main_exec_path = GetExecutablePath(argv0);
	if (main_exec_path.empty())
	{
		return res;
	}
	std::string exec_file_dir = llvm::sys::path::parent_path(main_exec_path).str();
	...
	res = exec_file_dir + ".release_info";
	return res;
}
...
const char DefaultChecks[] = // Enable these checks by default:
    "scope-diagnostic-*,"    //   * compiler diagnostics
...
int clangTidyMain(int argc, const char **argv) {
  ...
  clang::keyauth::CheckEnabledCheckerList(EnabledChecks, GetKeyGenFilePath(argv[0]), "clang-tidy");
  ...
  if (!ExportFixes.empty() && !Errors.empty()) {
	...
	size_t out_size = 0;
	llvm::raw_fd_ostream of(ExportFixes, EC, llvm::sys::fs::OF_None);
	if (EC) {
		llvm::errs() << "Error opening output file: " << EC.message() << '\n';
		return 1;
	}
	const char *ciptext = scope::encrypt::scope_encrypt_binary(OutString.c_str(), OutString.length(), &out_size);
	if (ciptext != NULL)
	{
		of.write(ciptext, out_size);
	}
    scope::encrypt::scope_free(ciptext);
	of.close();
  }
