...
public:
  ...
  void MacroDefined(const Token &MacroNameTok,
    ...
    // std::cout << MacroNameTok.getName() << std::endl;
...
static bool isKeywordAll(const Token &T) {
  // FIXME: better matching of keywords to avoid false positives.
  return T.isOneOf(tok::kw_auto, tok::kw_break, tok::kw_case, tok::kw_char,
                  tok::kw_const, tok::kw_continue, tok::kw_default, tok::kw_do,
                  tok::kw_double, tok::kw_else, tok::kw_enum, tok::kw_extern,
                  tok::kw_float, tok::kw_for, tok::kw_goto, tok::kw_if,
                  tok::kw_inline, tok::kw_int, tok::kw__ExtInt, tok::kw_long,
                  tok::kw_register, tok::kw_restrict, tok::kw_return, tok::kw_short,
                  tok::kw_signed, tok::kw_static, tok::kw_struct, tok::kw_switch,
                  tok::kw_typedef, tok::kw_union, tok::kw_unsigned, tok::kw_void,
                  tok::kw_volatile, tok::kw_while, tok::kw__Alignas, tok::kw__Alignof,
                  tok::kw__Atomic, tok::kw__Bool, tok::kw__Complex, tok::kw__Generic,
                  tok::kw__Imaginary, tok::kw__Noreturn, tok::kw__Static_assert, tok::kw__Thread_local,
                  tok::kw___func__ , tok::kw___objc_yes, tok::kw___objc_no, tok::kw___attribute,
                  tok::kw___builtin_choose_expr , tok::kw___builtin_offsetof, tok::kw___builtin_FILE,
                  tok::kw___builtin_FUNCTION , tok::kw___builtin_LINE, tok::kw___builtin_COLUMN);
}
...
void MacroParenthesesPPCallbacks::replacementList(const Token &MacroNameTok,
  ...
  if (possibleVarDecl(MI, MI->tokens_begin()))
    {
      // std::cout << "possibleVarDecl" << std::endl;
      Check->diag(MI->tokens_begin()->getLocation(), "Macro body is a variable declaration");
      return;
    }
  ...
  for (auto TI = MI->tokens_begin(), TE = MI->tokens_end(); TI != TE; ++TI) {
    ...
    if(Tok.isOneOf(tok::hash, tok::hashhash)) {
      Check->diag(Tok.getLocation(), "Macro body not allow # or ##");
    }
    if (isKeyword(Tok))
      return;
    ...
    if (Tok.isOneOf(tok::l_paren, tok::l_brace, tok::l_square)) {
      ...
      if (Count < 0)
        {
          Check->diag(Tok.getLocation(), "Macro body must be included by paren, brace, square.");
          return;
        }
...
void MacroParenthesesPPCallbacks::argument(const Token &MacroNameTok,
  ...
  if(isKeywordAll(MacroNameTok)) {
    Check->diag(MacroNameTok.getLocation(), "Macro not allow redefine reserved words.");
  }
