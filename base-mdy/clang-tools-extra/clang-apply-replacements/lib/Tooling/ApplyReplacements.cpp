...
std::error_code collectReplacementsFromDirectory(
  ...
  for (recursive_directory_iterator I(Directory, ErrorCode), E;
	...
	size_t plain_size;
	const char *plain_buff = clang::scope::encrypt::scope_decrypt_binary(Out.get()->getBufferStart(), Out.get()->getBufferSize(), &plain_size);
	if (plain_buff == NULL || plain_size <= 0)
	{
		if (plain_buff != NULL)
        {
            clang::scope::encrypt::scope_free(plain_buff);
        }
		continue;
	}
	std::unique_ptr<MemoryBuffer> DeOut = MemoryBuffer::getMemBufferCopy(llvm::StringRef(plain_buff, plain_size));
    clang::scope::encrypt::scope_free(plain_buff);

    yaml::Input YIn(DeOut.get()->getBuffer(), nullptr, &eatDiagnostics);
