...
public:
  ...
  APFloat &operator=(float floatNumber) {
    if (usesLayout<IEEEFloat>(*U.semantics)){
      U.IEEE = IEEEFloat(floatNumber);
    }
    if (usesLayout<DoubleAPFloat>(*U.semantics))
      U.Double.getFirst().U.IEEE = IEEEFloat(floatNumber);
    return *this;
  }
  ...
  void dump() const {
    if (usesLayout<IEEEFloat>(*U.semantics)) {
        std::cout << "APFloat dump: " << U.IEEE.convertToFloat() <<std::endl;
      }
    if (usesLayout<DoubleAPFloat>(*U.semantics))
      std::cout << "APFloat dump: " << U.Double.getFirst().U.IEEE.convertToFloat() << std::endl;
  }
