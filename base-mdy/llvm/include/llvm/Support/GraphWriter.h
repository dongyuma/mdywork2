...
public:
  ...
  //from llvm-cxxfilt
  std::string escapeDemangleString(const std::string &Mangled) {
	  int Status;
	  std::string Prefix;
      std::string escapeMangled = DOT::EscapeString(Mangled);

	  const char *DecoratedStr = escapeMangled.c_str();
	  size_t DecoratedLength = strlen(DecoratedStr);

	  char *Undecorated = nullptr;

	  if (((DecoratedLength >= 2 && strncmp(DecoratedStr, "_Z", 2) == 0) ||
	  ...
	  std::string Result(Undecorated ? Prefix + Undecorated : escapeMangled);
	  free(Undecorated);

      //html escape
      std::size_t pos = 0;
	  while ((pos < Result.size()) &&
          (std::string::npos != (pos = Result.find_first_of("\"&<>", pos))))
      {
          switch (Result[pos])
          {
		  case '\"':
  ...
  {
	  std::string escapeStr = escapeDemangleString(DTraits.getNodeLabel(Node, G));
      std::size_t pos = 0;
	  if ((pos = escapeStr.find(".") != std::string::npos) && (pos < escapeStr.size() - 1) && escapeStr[pos + 1] != '.')
	  {
		  return true;
      }
  ...
  void writeNode(NodeRef Node) {
    ...
    //by zhangchaoze 20220627 
    //Function node in callgraph uses function name without name decoration

    if (!DTraits.renderGraphFromBottomUp()) {

      //O << DOT::EscapeString(DTraits.getNodeLabel(Node, G));
      O << escapeDemangleString(DTraits.getNodeLabel(Node, G));  
      ...
      if (!NodeDesc.empty())
      {
		  ...
		  O << "|" << escapeDemangleString(NodeDesc);
      }
    }
    ...
    if (DTraits.hasEdgeDestLabels()) {
      ...
      for (; i != e && i != 64; ++i) {
        if (i) O << "|";
        //O << "<d" << i << ">" << DOT::EscapeString(DTraits.getEdgeDestLabel(Node, i));
        O << "<d" << i << ">" << escapeDemangleString(DTraits.getEdgeDestLabel(Node, i));
      }
