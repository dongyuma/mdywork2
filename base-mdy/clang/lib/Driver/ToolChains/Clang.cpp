...
void Clang::ConstructJob(Compilation &C, const JobAction &JA,
  ...
  std::string clang_exec_file_path = D.getClangProgramPath();
  std::string key_gen_file = llvm::sys::path::parent_path(clang_exec_file_path).str();
  if (!key_gen_file.empty())
  {
	  if (*(key_gen_file.end() - 1) != '/')
      {
          key_gen_file.append("/");
      }
      key_gen_file.append(".release_info");
  }

  std::string enable_analyze_prefix = "-analyzer-checker=";
  std::vector<std::string> enable_checks;
  for (auto Arg : Args.filtered(options::OPT_Xclang))
  {
	  Arg->claim();
	  std::string x_arg = StringRef(Arg->getValue()).str();
	  if (x_arg.find(enable_analyze_prefix) == 0)
      {
          std::string checker_name = x_arg.substr(enable_analyze_prefix.length());
          if (!checker_name.empty())
          {
              enable_checks.push_back(checker_name);
          }
      }
  }
  if (!enable_checks.empty())
  {
      clang::keyauth::CheckEnabledCheckerList(enable_checks, key_gen_file, "clangsa");
  }
  ...
  Args.AddLastArg(CmdArgs, options::OPT_femit_all_source_decls);
  Args.AddLastArg(CmdArgs, options::OPT_fhidden_out_project_decls);
