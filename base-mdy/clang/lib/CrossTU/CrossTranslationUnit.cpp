...
bool shouldImport(const VarDecl* VD, const ASTContext& ACtx) {
	...
	return VD->getType().isTrivialType(ACtx);
}

static bool hasBodyOrInit(const FunctionDecl *D, const FunctionDecl *&DefD) {
  return D->hasBody(DefD);
}
//此处修改为了数组全局变量没有初始化的跨翻译单元检测，后期需要再此处重新设计更好的ctu结构
static bool hasBodyOrInit(const VarDecl *D, const VarDecl *&DefD) {
    DefD = D;
    return D->getAnyInitializer(DefD) || D->getType()->isArrayType();
}
...
//使用ASTUnitStorage类里的hasEquivalentFunctionName方法判断有没有和LookupName函数相似的函数
bool
CrossTranslationUnitContext::hasApproachFunctionDecl(StringRef LookupName,
    StringRef CrossTUDir,
    StringRef IndexName,
    bool DisplayCTUProgress) 
{
    bool res = ASTStorage.hasApproachFunction(
...
llvm::Expected<const T *> CrossTranslationUnitContext::getCrossTUDefinitionImpl(
    ...
    bool DisplayCTUProgress, bool notImport) {
...
c:@F@HDY#S#i#
按照#切割
判断第一块c:@F@HDY函数名和函数类型必须相等
判断块数相等，这代表参数的数量一样
*/
static bool isSimilarLookupName(StringRef name1, StringRef name2)
{
	SmallVector<StringRef, 10> name1_v, name2_v;
    name1.split(name1_v, '#');
    name2.split(name2_v, '#');
	if (name1_v.empty() || name2_v.empty())
    {
        return false;
    }
	if (name1_v.size() == name2_v.size() &&
		*(name1_v.begin()) == *(name2_v.begin()))
    {
        return true;
    }
    return false;
...
//加载ast索引文件，遍历上面每一条信息
bool
CrossTranslationUnitContext::ASTUnitStorage::hasApproachFunction(
    ...
    bool DisplayCTUProgress) 
{
    ...
    for (auto iter = NameFileMap.begin(); iter != NameFileMap.end(); iter++)
    {
		if (isSimilarLookupName((*iter).getKeyData(), FunctionName))
        {
            return true;
        }
    }
