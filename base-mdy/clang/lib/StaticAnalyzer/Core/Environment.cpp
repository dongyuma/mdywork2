...
SVal Environment::getSVal(const EnvironmentEntry &Entry,
  ...
  //     if(const auto *DV = cast<VarDecl>(D))
  //       return getSVal(EnvironmentEntry(DV->getInit(), LCtx), svalBuilder);
  //   return UndefinedVal();
  // }

  case Stmt::ImplicitCastExprClass: {
    const auto *IC = cast<ImplicitCastExpr>(S);
    if(IC->getCastKind()==CK_IntegralCast)
    {
      if(IC->getType().getAsString()=="unsigned int" || IC->getType().getAsString()=="const unsigned int")
      {
        if(const auto *SE = dyn_cast<IntegerLiteral>(IC->getSubExpr()))
          return svalBuilder.makeIntVal(SE->getValue(), true);
      }
      // if(IC->getType().getAsString()
    } else if(IC->getCastKind()==CK_IntegralToFloating) {
      if(IC->getType().getAsString()=="double") {
        if(const auto *SE = dyn_cast<IntegerLiteral>(IC->getSubExpr()))
          return svalBuilder.makeFloatVal(*(SE->getValue().getRawData()));
      }
    }

    // if(const auto *SE = IC->getSubExpr())
    //   return getSVal(EnvironmentEntry(SE, LCtx), svalBuilder);
