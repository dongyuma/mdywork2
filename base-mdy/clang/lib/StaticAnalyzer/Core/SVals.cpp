...
bool SVal::isLTZeroConstant() const {
  if (Optional<loc::ConcreteInt> LV = getAs<loc::ConcreteInt>())
    return LV->getValue() < 0;
  if (Optional<nonloc::ConcreteInt> NV = getAs<nonloc::ConcreteInt>())
                                      ...
                                      const ConcreteFloat &R) const
{
	const llvm::APFloat *X =
		svalBuilder.getBasicValueFactory().evalAPFloat(Op, getValue(), R.getValue());
...
nonloc::ConcreteFloat::evalMinus(SValBuilder &svalBuilder) const {
  return svalBuilder.makeFloatVal(-getValue().convertToFloat());
}
...
SVal loc::ConcreteFloat::evalBinOp(BasicValueFactory &BasicVals, 
                                   BinaryOperator::Opcode Op, 
                                   const ConcreteFloat &R) const
{
	assert(BinaryOperator::isComparisonOp(Op) || Op == BO_Sub);

	const llvm::APFloat *X = BasicVals.evalAPFloat(Op, getValue(), R.getValue());
...
void SVal::dumpToStream(raw_ostream &os) const {
  // std::cout << "In the dumpToStream" << std::endl;
  // std::cout << "getBaseKind " << getBaseKind() << endl;
  // std::cout << "UnknownValKind " << UnknownValKind << endl;
  // std::cout << "NonLocKind " << NonLocKind << endl;
  // std::cout << "LocKind " << LocKind << endl;
  // std::cout << "UndefinedValKind " << UndefinedValKind << endl;
  switch (getBaseKind()) {
    case UnknownValKind:
      os << "Unknown";
      ...
      break;
    case NonLocKind:
      // std::cout << "NonLoc" << std::endl;
      castAs<NonLoc>().dumpToStream(os);
      // std::cout << "After NonLoc" << std::endl;
      break;
    case LocKind:
    ...
    case UndefinedValKind:
      // std::cout << "UndefinedValKind" << std::endl;
      os << "Undefined";
      break;
  }
  // std::cout << "after the dumpToStream" << std::endl;
}


void NonLoc::dumpToStream(raw_ostream &os) const {
  // std::cout << "In the NonLoc dumpToStream" << std::endl;
  // std::cout << "getSubKind " << getSubKind() << std::endl;
  // std::cout << "ConcreteIntKind " << nonloc::ConcreteIntKind << std::endl;
  // std::cout << "ConcreteFloatKind " << nonloc::ConcreteFloatKind << std::endl;
  // std::cout << "SymbolValKind " << nonloc::SymbolValKind << std::endl;
  switch (getSubKind()) {
    case nonloc::ConcreteIntKind: {
      // std::cout << "ConcreteIntKind" << std::endl;
    ...
    case nonloc::ConcreteFloatKind : {
      // std::cout << "ConcreteFloatKind" << std::endl;
      const auto &Value = castAs<nonloc::ConcreteFloat>().getValue();
      // std::cout << "ConcreteFloat Value: " << Value.convertToFloat() << std::endl;
      os << Value.convertToFloat();
      break;
    }
    case nonloc::SymbolValKind:
      // std::cout << "SymbolValKind" << std::endl;
      os << castAs<nonloc::SymbolVal>().getSymbol();
      break;

    case nonloc::LocAsIntegerKind: {
      // std::cout << "LocAsIntegerKind" << std::endl;
      const nonloc::LocAsInteger& C = castAs<nonloc::LocAsInteger>();
      os << C.getLoc() << " [as " << C.getNumBits() << " bit integer]";
      break;
    }
    case nonloc::CompoundValKind: {
      // std::cout << "CompoundValKind" << std::endl;
    ...
    case nonloc::LazyCompoundValKind: {
      // std::cout << "LazyCompoundValKind" << std::endl;
    ...
    case nonloc::PointerToMemberKind: {
      // std::cout << "PointerToMemberKind" << std::endl;
      os << "pointerToMember{";
