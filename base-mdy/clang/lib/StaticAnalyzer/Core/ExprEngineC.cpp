...
void ExprEngine::VisitDeclStmt(const DeclStmt *DS, ExplodedNode *Pred,
                               ExplodedNodeSet &Dst, ProgramStateRef &state) {
  ...
  if (const Expr *InitEx = VD->getInit()) {
    if(isa<InitListExpr>(InitEx)) {
      const InitListExpr *IE = dyn_cast<InitListExpr>(InitEx);
      ...
      if (!IE->isGLValue() && !IE->isTransparent() &&
        ...
        for (InitListExpr::const_reverse_iterator it = IE->rbegin(),
            ei = IE->rend(); it != ei; ++it) {
          SVal V = state->getSVal(cast<Expr>(*it), LC);
          vals = getBasicVals().prependSVal(V, vals);
        }
        // state = state->BindExpr(IE, LC, svalBuilder.makeCompoundVal(T, vals));
        state = state->bindLoc(state->getLValue(VD, LC).castAs<Loc>(),
                          svalBuilder.makeCompoundVal(T, vals), LC, true);
      }
    } else {
