...
SVal SimpleSValBuilder::evalCastFromNonLoc(NonLoc val, QualType castTy) {
  ...
  // If value is a non-integer constant or non-Float constant, produce unknown.
  ...
  if (castTy->isBooleanType()) {
      ...
      {
          float f = val.castAs<nonloc::ConcreteFloat>().getValue().convertToFloat();
          bool b = true;
  ...
  if (!isLocType && !castTy->isIntegralOrEnumerationType() && !castTy->isFloatingType())
    return UnknownVal();
  ...
  if (castTy->isIntegralOrEnumerationType())
  {
	  ...
	  case nonloc::ConcreteFloatKind:
	  {
		  llvm::APFloat f = val.castAs<nonloc::ConcreteFloat>().getValue();
		  llvm::APSInt i;
          bool IsExact = false;
		  BasicVals.getAPSIntType(castTy).apply(i);
		  bool Overflows = f.convertToInteger(i, llvm::APFloat::rmTowardZero, &IsExact) & llvm::APFloat::opInvalidOp;
  ...
  else if (castTy->isFloatingType())
  {
	  ...
	  case nonloc::ConcreteIntKind:
	  {
		  ...
		  if (isLocType)
		  {
			  return makeFloatLocVal(llvm::APFloat((float)i.getExtValue()));
		  }
		  else
		  {
			  return makeFloatVal(i.getExtValue());
		  }
	  }
	  case nonloc::ConcreteFloatKind:
	  {
...
SVal SimpleSValBuilder::evalBinOpNN(ProgramStateRef state,
  ...
  while (1) {
    ...
    case nonloc::ConcreteFloatKind: {
      ...
      if (rhs.getSubKind() == nonloc::ConcreteFloatKind) {
        llvm::APFloat RHSValue = rhs.castAs<nonloc::ConcreteFloat>().getValue();
        if (BinaryOperator::isComparisonOp(op)) {
        ...
        if (!Result)
          return UndefinedVal();
        return nonloc::ConcreteFloat(*Result);
      }
      return UnknownVal();
    }
    case nonloc::ConcreteIntKind: {
