...
public:
  ...
  StoreRef getInitialStore(const LocationContext *InitLoc) override {
    ...
    if (const auto *FD = dyn_cast<FunctionDecl>(InitLoc->getDecl()))
        IsMainAnalysis = FD->isMain(); //&& !Ctx.getLangOpts().CPlusPlus; //modify by zhangchaoze 20220828 allow c++
...
public: // Part of public interface to class.
  ...
  StoreRef BindDefaultInitial(Store store, const MemRegion *R,
    ...
    //修复绑定建模影响不可达分支的bug 
    return StoreRef(bind(B, loc::MemRegionVal(R), V).asStore(), *this);
    //B = B.addBinding(BindingKey::Make(R, BindingKey::Direct), V);
  ...
  StoreRef BindDefaultZero(Store store, const MemRegion *R) override {
    ...
    //修复绑定建模影响不可达分支的bug (可能和isMainAnalysis有关)
	return StoreRef(B.asStore(), *this);
  }
...
SVal RegionStoreManager::getBinding(RegionBindingsConstRef B, Loc L, QualType T) {
  ...
  // std::cout << "In the getBinding ++++" << std::endl;
  ...
  if (L.getAs<loc::ConcreteInt>()) {
    // std::cout << "In the getBinding ++++ Int" << std::endl;
    return UnknownVal();
  }
  if (L.getAs<loc::ConcreteFloat>()) {
    // std::cout << "In the getBinding ++++ Float" << std::endl;
    return UnknownVal();
  }
  ...
  // std::cout << "In the getBinding ++++ MemRegionVal" << std::endl;
  ...
  // std::cout << "Before the getBinding ++++ ComplexType" << std::endl;
  if (RTy->isAnyComplexType()){
    // std::cout << "In the getBinding ++++ ComplexType" << std::endl;
    return UnknownVal();
  }
  ...
  if (RTy->isVectorType())
    return UnknownVal();
  // std::cout << "After the getBinding ++++ isVectorType" << std::endl;
  if (const FieldRegion* FR = dyn_cast<FieldRegion>(R))
  ...
  // std::cout << "After the getBinding ++++ ObjCIvarRegion" << std::endl;
  if (const VarRegion *VR = dyn_cast<VarRegion>(R)) {
    ...
    // CastRetrievedVal(getBindingForVar(B, VR), VR, T).dump();
    // std::cout << "Before the getBinding ++++ getBindingForVar" << std::endl;
    return CastRetrievedVal(getBindingForVar(B, VR), VR, T);
  }
  // std::cout << "After the getBinding ++++ VarRegion" << std::endl;
  const SVal *V = B.lookup(R, BindingKey::Direct);

  // Check if the region has a binding.
  if (V)
    return *V;
  // std::cout << "After the getBinding ++++ V" << std::endl;
  ...
  // std::cout << "After the getBinding ++++ hasStackNonParametersStorage" << std::endl;
