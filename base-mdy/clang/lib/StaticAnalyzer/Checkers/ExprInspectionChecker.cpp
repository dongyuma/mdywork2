...
ExplodedNode *ExprInspectionChecker::reportBug(llvm::StringRef Msg,
  ...
  if (!BT)
    BT.reset(new BugType(this, "检查分析器假设","调试"));
...
void ExprInspectionChecker::analyzerDenote(const CallExpr *CE,
                                           CheckerContext &C) const {
  if (CE->getNumArgs() < 2) {
    reportBug("clang_analyzer_denote（）需要一个符号和一个字符串文字",
              C);
    return;
  }
...
void ExprInspectionChecker::analyzerExpress(const CallExpr *CE,
                                            CheckerContext &C) const {
  if (CE->getNumArgs() == 0) {
    reportBug("clang_analyzer_express（）需要一个符号", C);
    return;
  }
...
void ExprInspectionChecker::analyzerIsTainted(const CallExpr *CE,
                                              CheckerContext &C) const {
  if (CE->getNumArgs() != 1) {
    reportBug("clang_analyzer_isTainted()只需要一个参数", C);
    return;
  }
