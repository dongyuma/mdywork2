...
class StreamChecker : public Checker<check::PreCall, eval::Call,
  ...
  BugType BT_FileNull{this, "空流指针", "流处理错误"};
  BugType BT_UseAfterClose{this, "封闭流", "流处理错误"};
  BugType BT_UseAfterOpenFailed{this, "无效流",
                                "流处理错误"};
  BugType BT_IndeterminatePosition{this, "流状态无效",
                                   "流处理错误"};
  BugType BT_IllegalWhence{this, "非法whence参数",
                           "流处理错误"};
  BugType BT_StreamEof{this, "流已在文件结尾", "流处理错误"};
  BugType BT_ResourceLeak{this, "资源泄漏", "流处理错误",
                          /*SuppressOnSink =*/true};
...
void StreamChecker::reportFEofWarning(CheckerContext &C,
  ...
  if (ExplodedNode *N = C.generateNonFatalErrorNode(State)) {
    C.emitReport(std::make_unique<PathSensitiveBugReport>(
        BT_StreamEof,
        "当流处于EOF（文件结尾）状态时调用Read（读取）函数。函数无效。",
        N));
    return;
  }
  C.addTransition(State);
