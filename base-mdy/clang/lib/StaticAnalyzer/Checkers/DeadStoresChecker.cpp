...
public:
  ...
  bool isLive(const LiveVariables::LivenessValues &Live, const VarDecl *D) {
    ...
    // to analyze that yet.
    //cout<<InEH->count(D)<<endl;
    return InEH->count(D);
  }

  bool isSuppressed(SourceRange R) {
    //cout<<"進入issuppressed"<<endl;
  ...
  void Report(const VarDecl *V, DeadStoreKind dsk,
    ...
    switch (dsk) {
      ...
      case Enclosing:
        ...
        BugType = "嵌套分配无效";
        os << "尽管存储到变量" << *V
           << "的值在闭环的表达式中被使用，但是这个值实际没有被用到";
        break;
    }

    BR.EmitBasicReport(AC->getDecl(), Checker, BugType, "存储无效", os.str(),
                       L, R, Fixits);
  }

  //检查变量的声明语句是否是deadcode
  ...
  void observeStmt(const Stmt *S, const CFGBlock *block,
    ...
    if (const BinaryOperator* B = dyn_cast<BinaryOperator>(S)) {
      //cout<<"進入Binary"<<endl;
      if (!B->isAssignmentOp()) return; // Skip non-assignments.跳过不是分配符的操作符
