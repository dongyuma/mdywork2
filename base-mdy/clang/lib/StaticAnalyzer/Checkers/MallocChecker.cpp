...
void MallocChecker::checkPostCall(const CallEvent &Call,
                                  CheckerContext &C) const {
  // cout<<"进入mallocchecker的checkPostCall函数"<<endl;
  if (C.wasInlined)
    return;
...
void MallocChecker::HandleOffsetFree(CheckerContext &C, SVal ArgVal,
  ...
  if (!BT_OffsetFree[*CheckKind])
    BT_OffsetFree[*CheckKind].reset(new BugType(
        CheckNames[*CheckKind], "无偏移", categories::MemoryError));
  ...
  if (ExplodedNode *N = C.generateErrorNode()) {
    if (!BT_UseFree[*CheckKind])
      BT_UseFree[*CheckKind].reset(new BugType(
          CheckNames[*CheckKind], "释放后使用", categories::MemoryError));
  ...
  if (ExplodedNode *N = C.generateErrorNode()) {
    if (!BT_DoubleFree[*CheckKind])
      BT_DoubleFree[*CheckKind].reset(new BugType(
          CheckNames[*CheckKind], "同一个指针释放两次", categories::MemoryError));

    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT_DoubleFree[*CheckKind],
        (Released ? "尝试释放已经被释放的内存"
                  : "尝试释放不拥有的内存"),
        N);
    R->addRange(Range);
    R->markInteresting(Sym);
...
void MallocChecker::HandleDoubleDelete(CheckerContext &C, SymbolRef Sym) const {
  ...
  if (ExplodedNode *N = C.generateErrorNode()) {
    ...
    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT_DoubleDelete, "尝试删除已经被释放的内存", N);
  ...
  if (ExplodedNode *N = C.generateErrorNode()) {
    if (!BT_UseZerroAllocated[*CheckKind])
      BT_UseZerroAllocated[*CheckKind].reset(
          new BugType(CheckNames[*CheckKind], "零分配的使用",
                      categories::MemoryError));

    auto R = std::make_unique<PathSensitiveBugReport>(
        *BT_UseZerroAllocated[*CheckKind], "零分配的内存的使用", N);
...
void MallocChecker::HandleFunctionPtrFree(CheckerContext &C, SVal ArgVal,
  ...
  if (ExplodedNode *N = C.generateErrorNode()) {
    if (!BT_BadFree[*CheckKind])
      BT_BadFree[*CheckKind].reset(new BugType(
          CheckNames[*CheckKind], "错误释放", categories::MemoryError));
...
void MallocChecker::HandleLeak(SymbolRef Sym, ExplodedNode *N,
  ...
  if (!BT_Leak[*CheckKind]) {
    ...
    BT_Leak[*CheckKind].reset(new BugType(CheckNames[*CheckKind], "内存泄漏",
                                          categories::MemoryError,
                                          /*SuppressOnSink=*/true));
  }
...
void MallocChecker::checkPreCall(const CallEvent &Call,
                                 CheckerContext &C) const {
  // cout<<"进入mallocchecker的checkpreCall函数"<<endl;
