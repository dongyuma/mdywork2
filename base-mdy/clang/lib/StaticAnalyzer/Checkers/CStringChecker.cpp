...
public:
  ...
  ProgramStateRef CheckLocationEqual(CheckerContext &C,
                                    ProgramStateRef state,
                                    const Expr *S, SVal l,
                                    const char *message = nullptr) const;

  ProgramStateRef CheckBufferAccessEqual(CheckerContext &C,
                                        ProgramStateRef state,
                                        const Expr *Size,
                                        const Expr *FirstBuf,
...
ProgramStateRef CStringChecker::CheckLocationEqual(CheckerContext &C,
                                             ProgramStateRef state,
                                             const Expr *S, SVal l,
  ...
  DefinedSVal ValOne = svalBuilder.makeIntVal(1, S->getType());

  SVal IdxPlusOne = svalBuilder.evalBinOpNN(state, BO_Add, Idx.castAs<NonLoc>(), ValOne.castAs<NonLoc>(), S->getType());

  ProgramStateRef StEqual = state->assume(svalBuilder.evalEQ(state, Size, IdxPlusOne).castAs<DefinedOrUnknownSVal>(), true);
  ProgramStateRef StNotEqual = state->assume(svalBuilder.evalEQ(state, Size, IdxPlusOne).castAs<DefinedOrUnknownSVal>(), false);
  ...
  if (!StEqual) {
    ...
    // Emit a bug report.
    ErrorMessage Message =
        createOutOfBoundErrorMsg(CurrentFunctionDescription, AccessKind::write);
    emitOutOfBoundsBug(C, StOutBound, S, Message);
    return nullptr;
  }
  ...
  // if(!StEqual){
  //   ExplodedNode *N = C.generateErrorNode(StEqual);
  //   if (!N)
  //     return nullptr;
  ...
  //   // Name = "Memset do not completely";
  //   if (!BT_Bounds) {
  //     BT_Bounds.reset(new BuiltinBug(
  //         Name, "memset not complete",
  //         "memset function need set more space of memory."));
  //   }
...
ProgramStateRef CStringChecker::CheckBufferAccessEqual(CheckerContext &C,
                                                 ProgramStateRef state,
                                                 const Expr *Size,
                                                 const Expr *FirstBuf,
  ...
  AnyArgExpr aExpr = {FirstBuf, 0};
  state = checkNonNull(C, state, aExpr, BufVal);
  if (!state)
    return nullptr;
  ...
  if (Optional<Loc> BufLoc = BufStart.getAs<Loc>()) {
    ...
    SVal BufEnd = svalBuilder.evalBinOpLN(state, BO_Add, *BufLoc,
                                          LastOffset, PtrTy);
    state = CheckLocationEqual(C, state, warningExpr, BufEnd, firstMessage);
...
void CStringChecker::emitOverlapBug(CheckerContext &C, ProgramStateRef state,
  ...
  if (!BT_Overlap)
    BT_Overlap.reset(new BugType(Filter.CheckNameCStringBufferOverlap,
                                 categories::UnixAPI, "错误参数"));

  // Generate a report for this bug.
  auto report = std::make_unique<PathSensitiveBugReport>(
      *BT_Overlap, "参数不能是重叠的缓冲区", N);
...
void CStringChecker::evalMemset(CheckerContext &C, const CallExpr *CE) const {
  ...
  State = CheckBufferAccessEqual(C, State, CE->getArg(2), CE->getArg(0));
  if (!State)
    return;
