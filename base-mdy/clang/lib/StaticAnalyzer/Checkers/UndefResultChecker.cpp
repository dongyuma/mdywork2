...
static bool isShiftOverflow(const BinaryOperator *B, CheckerContext &C) {
  return C.isGreaterOrEqualStrictMode(
...
void UndefResultChecker::checkPostStmt(const BinaryOperator *B,
  ...
  if (C.getSVal(B).isUndef()) {
    ...
    if (!BT)
      BT.reset(
          new BuiltinBug(this, "操作结果是无效的垃圾或未定义的"));
    ...
    if (Ex) {
      ...
      if ((B->getOpcode() == BinaryOperatorKind::BO_Shl ||
        ...
        if (!I)
          OS << "a value that maybe";
        else if (I->isUnsigned())
          OS << '\'' << I->getZExtValue() << "\', which maybe";
        else
          OS << '\'' << I->getSExtValue() << "\', which maybe";
           ...
           B->getOpcode() == BinaryOperatorKind::BO_Shr)
  {
    ...
    const Expr *Ex = B->getLHS()->IgnoreParenCasts();
    if (Ex) {
      report->addRange(Ex->getSourceRange());
      bugreporter::trackExpressionValue(N, Ex, *report);
    }
