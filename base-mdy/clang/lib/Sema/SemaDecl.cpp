...
bool Sema::MergeFunctionDecl(FunctionDecl *New, NamedDecl *&OldD,
  ...
  //此处修改为了发生这种错误后，csa引擎继续工作。因此将这种错误的级别从err调整成warning级别
  Diag(New->getLocation(), diag::warn_conflicting_types) << New->getDeclName();
  Diag(OldLocation, PrevDiag) << Old << Old->getType();
  return false;
}
...
static StorageClass getFunctionStorageClass(Sema &SemaRef, Declarator &D) {
  ...
  case DeclSpec::SCS_private_extern: return SC_PrivateExtern;
   case DeclSpec::SCS_interrupt: return SC_Interrupt;
  }
...
void Sema::CheckMain(FunctionDecl* FD, const DeclSpec& DS) {
  ...
  if (HasExtraParameters) {
    //close main ExtraParameters error diag by zhangchaoze 20220819
