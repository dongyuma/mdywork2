...
//===----------------------------------------------------------------------===//
//
//  This file implements the Diagnostic-related interfaces. 诊断相关接口
//
//===----------------------------------------------------------------------===//
...
FormatDiagnostic(const char *DiagStr, const char *DiagEnd,
  ...
  if (DiagEnd - DiagStr == 2 &&
    ...
    //Fixed: 2021-06-23 by zhangchaoze 修复无法输出中文字符的bug
    //UTF-8编码有可能是多字节的 下面这种方法使用isPrint是有问题的
    //后续可优化检测是否是UTF-8可现实编码
    OutStr.append(S.begin(), S.end()); //使用原始字符串
    /*for (char c : S) {
