...
SourceManager::SourceManager(DiagnosticsEngine &Diag, FileManager &FileMgr,
  ...
  llvm::Optional<std::string> proj_env = llvm::sys::Process::GetEnv("CHECKSCOPE_ANALYZE_PROJECT_DIR");
  if (proj_env)
  {
	  AnalyzeProjectDir = proj_env.getValue();
  }
  if (!AnalyzeProjectDir.empty())
  {
	  if (*(AnalyzeProjectDir.end() - 1) != '/')
	  {
          AnalyzeProjectDir.append("/");
	  }
	  SmallString<128> nv_path = StringRef(AnalyzeProjectDir);
	  llvm::sys::path::native(nv_path);
      AnalyzeProjectDir = nv_path.str().str();
  }
}
...
const char *SourceManager::getCharacterLine(FullSourceLoc FSL,
                                            bool *Invalid) const {
  // Note that this is a hot function in the getSpelling() path, which is
  // heavily used by -E mode.
  std::pair<FileID, unsigned> LocInfo = getDecomposedSpellingLoc(FSL);
  ...
  return Buffer ? Buffer->getBufferStart() + LocInfo.second - columnNumber + 1
                : "<<<<INVALID BUFFER>>>>";
}
...
/// Returns whether the PresumedLoc for a given SourceLocation is
/// in the analyze project dir (including subdirectories).
///
/// This check ignores line marker directives.
/// If the project directory is not set, check whether SourceLocation is 
/// under the main file directory (including subdirectories)
bool SourceManager::isInProjectFile(SourceLocation Loc) const
{
    std::string prefix_path = AnalyzeProjectDir;
    SmallString<128> nv_loc_path;
	if (const FileEntry *F = getFileEntryForID(getFileID(Loc)))
    {
        nv_loc_path = F->tryGetRealPathName();
    }
	llvm::sys::path::native(nv_loc_path);
    if (nv_loc_path.empty())
    {
    ...
    {
        SmallString<128> nv_main_path;
		if (const FileEntry *F = getFileEntryForID(getMainFileID()))
        {
            nv_main_path = llvm::sys::path::parent_path(F->tryGetRealPathName());
            llvm::sys::path::native(nv_main_path);
            prefix_path = nv_main_path.str().str();
        }
		if (!prefix_path.empty() && *(prefix_path.end() - 1) != '/')
		{
			prefix_path.append("/");
		}
    }
    ...
    return nv_loc_path.startswith(prefix_path);
}
