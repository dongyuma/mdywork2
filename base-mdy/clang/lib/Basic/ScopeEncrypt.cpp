...
#include "clang/Basic/ScopeEncrypt.h"
...
uint8_t __IC__[] = { 0x43,0x48,0x45,0x43,0x4b,0x53,0x43,0x4f,0x50,0x45,0x5f,0x49,0x4e,0x53,0x49,0x44,0x45,0x5f,0x43,0x48,0x45,0x43,0x4b,0x00 };
uint8_t __IK__[] = { 0x43,0x68,0x65,0x63,0x6b,0x53,0x63,0x6f,0x70,0x65,0x00 };
uint8_t __k__[] = { 0x10, 0x09, 0x57, 0x7f, 0x88, 0xae, 0xdc, 0x3f, 0xcf, 0xfe, 0xdf, 0x73, 0xe9, 0x85, 0x44, 0x3b };

//#define NOT_SCOPE_CRYPT //当放开该宏时加解密为原值拷贝(即不进行加解密)
...
static uint8_t *AES_CBC_decrypt_buffer(struct AES_ctx *ctx, const uint8_t *data, size_t *data_length)
{
	...
	res = (uint8_t *)calloc((size_t)plain_len, sizeof(uint8_t));
	if (res == NULL)
	{
		free(buff);
		return NULL;
	}
	memcpy(res, buff + (2 * sizeof(uint64_t)), (size_t)plain_len);
	free(buff);
	*data_length = (size_t)plain_len;
	return res;
}
const char *scope_k_encrypt_hex(const char *plain_data, size_t data_size, const uint8_t key[16])
{
	...
	ciphertext = AES_CBC_encrypt_buffer(&ctx, (const uint8_t *)plain_data, &ciphertext_len);
	if (ciphertext == NULL)
	{
		return NULL;
	}
...
#ifndef NOT_SCOPE_CRYPT
	char *plaintext = NULL;
	size_t ciphertext_len = in_size;
	uint8_t iv[] = { 0x93, 0x19, 0x09, 0x17, 0x15, 0x20, 0x22, 0x29, 0x10, 0x68, 0x69, 0x15, 0xec, 0xcd, 0xef, 0x78 };
	struct AES_ctx ctx;
	AES_init_ctx_iv(&ctx, key, iv);
	plaintext = (char *)AES_CBC_decrypt_buffer(&ctx, (const uint8_t *)ciphers_data, &ciphertext_len);
	if (plaintext == NULL)
	{
		return NULL;
	}
...
{
	return scope_k_encrypt_binary(plain_data, in_size, out_size, __k__);
}
const char *scope_decrypt_binary(const char *ciphers_data, size_t in_size, size_t *out_size)
{
	return scope_k_decrypt_binary(ciphers_data, in_size, out_size, __k__);
}
