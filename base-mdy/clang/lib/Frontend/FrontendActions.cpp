...
EmitBiniouASTAction::CreateASTConsumer(CompilerInstance &CI, StringRef InFile)
{
    ...
    return CreateASTExporter(CI, ASTExporterOutputFormat::AEOF_BINIOU, std::move(OS));
}

std::unique_ptr<clang::ASTConsumer> 
EmitYojsonASTAction::CreateASTConsumer(CompilerInstance &CI, StringRef InFile)
{
	std::unique_ptr<raw_pwrite_stream> OS =
		CI.createOutputFile(CI.getFrontendOpts().OutputFile, /*Binary=*/true,
			/*RemoveFileOnSignal=*/true, InFile,
	...
	return CreateASTExporter(CI, ASTExporterOutputFormat::AEOF_YOJSON, std::move(OS));
}

std::unique_ptr<clang::ASTConsumer> 
EmitJsonASTAction::CreateASTConsumer(CompilerInstance &CI, StringRef InFile)
{
	std::unique_ptr<raw_pwrite_stream> OS =
		CI.createOutputFile(CI.getFrontendOpts().OutputFile, /*Binary=*/true,
			/*RemoveFileOnSignal=*/true, InFile,
	...
	return CreateASTExporter(CI, ASTExporterOutputFormat::AEOF_JSON, std::move(OS));
}
