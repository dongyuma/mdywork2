...
///
/// \file
/// Implements # directive processing for the Preprocessor. 预处理指令处理
///
//===----------------------------------------------------------------------===//
...
void Preprocessor::HandleDirective(Token &Result) {
  ...
  //增加回调触发的接口，当匹配到#号时，调用ppcallback里我们新加的check接口InvalidPreprocessingDirective
  if (Callbacks)
    Callbacks->InvalidPreprocessingDirective(Result);
  CurPPLexer->ParsingPreprocessorDirective = true;
  if (CurLexer) CurLexer->SetKeepWhitespaceMode(false);
...
void Preprocessor::HandleUndefDirective() {
  ...
  // Check to see if this is the last token on the #undef line./检查这是否是#undef行的最后一个标记。
