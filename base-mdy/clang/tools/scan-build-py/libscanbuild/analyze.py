...
def parse_commands_json(path):
    if os.path.exists(path + "/compile_commands.json"):
        with open(path + "/compile_commands.json") as f:
            files = json.load(f)
            return files
    else:
        return none
...
def run_tidy_check(path):
    base_path = path.split('/scan-result')[0]
    files = parse_commands_json(base_path)
    fo = open("/tmp/result-path", "w")
    fo.write(path)
    fo.close()
    if files:
        for file in files:
            command = 'clang-tidy -checks=\'gb5369-StructInit\' -p %s %s' % (file['directory'], file['file'])
            output = os.popen(command)
            for one in output.readlines():
                print(one)
