...
def assemble_cover(args, prefix, fragments):
    ...
    if args.html_title is None:
        args.html_title = os.path.basename(prefix) + '分析报告'
    ...
    with open(os.path.join(args.output, 'index.html'), 'w') as handle:
        ...
        |    <h1>{html_title}</h1>
        |    <table>
        |      <tr><th>用户名:</th><td>{user_name}@{host_name}</td></tr>
        |      <tr><th>分析目录:</th><td>{current_dir}</td></tr>
        |      <tr><th>命令行:</th><td>{cmd_args}</td></tr>
        |      <tr><th>Clang版本:</th><td>{clang_version}</td></tr>
...
def bug_summary(output_dir, bug_counter):
    ...
    with open(name, 'w') as handle:
        ...
        |      <td>Bug类型</td>
        |      <td>数量</td>
        |      <td class="sorttable_nosort">是否展示?</td>
        |    </tr>
        |  </thead>
        |  <tbody>""", indent))
        handle.write(reindent("""
        |    <tr style="font-weight:bold">
        |      <td class="SUMM_DESC">所有Bug</td>
        |      <td class="Q">{0}</td>
        |      <td>
        |        <center>
...
def bug_report(output_dir, prefix):
    ...
    with open(name, 'w') as handle:
        ...
        |      <td><a href="{report_file}#EndPath">查看报告</a></td>
