...
void MapExtDefNamesConsumer::handleDecl(const Decl *D) {
  ...
  if (const auto *FD = dyn_cast<FunctionDecl>(D)) {
    ...
    //修改引擎过滤条件，增加非const类型全局变量的导出
    if (cross_tu::shouldImport(VD, Ctx) && (VD->hasInit()||(VD->getType()->isArrayType() && !VD->hasExternalStorage())))
    { 
      if (const Expr* Init = VD->getInit())
      {
        addIfInMain(VD, Init->getBeginLoc());
      }
      else if (VD->getType()->isArrayType() && !VD->hasExternalStorage())
      {
        addIfInMain(VD, VD->getLocation());
      }
    }
