 ...
 * LineFeed: Unix LF
 *
 * Describe: 导出AST为biniou yojson json格式文件
 */

#ifndef LLVM_CLANG_AST_ASTEXPORTER_H
#define LLVM_CLANG_AST_ASTEXPORTER_H
#include <memory>
...
#include "ASTExport/NamePrinter.h"
#include "ASTExport/ATDWriter.h"

namespace astexport{

class ASTExporterOptions
{
public:
	...
	ATDWriter::ATDWriterOptions atdWriterOptions = { false, true };

public:
    ASTExporterOptions(const CompilerInstance &CI)
    {
		inputFile = CI.getFrontendOpts().Inputs[0];
        outputFile = CI.getFrontendOpts().OutputFile;
        const FileEntry *FE = CI.getSourceManager().getFileEntryForID(CI.getSourceManager().getMainFileID());
        if (FE)
        {
            inputPath = FE->tryGetRealPathName().str();
        }
        else
        {
            inputPath = inputFile.getFile().str();
        }
        SmallString<128> nv_path = StringRef(inputPath);
        llvm::sys::path::native(nv_path);
        inputPath = nv_path.c_str();
    }

	const std::string normalizeSourceFilePath(ASTContext &AC, FileID FID) const
    {
        std::string res;
        if (const FileEntry *F = AC.getSourceManager().getFileEntryForID(FID))
        {
            res = F->tryGetRealPathName().str();
        }
        return res;
    }
    ...
    {
        return normalizeSourcePath(StringRef(path));
    }

    const std::string normalizeSourcePath(llvm::StringRef path) const
    {
		SmallString<128> nv_fd_path = path;
		llvm::sys::path::native(nv_fd_path);
		return nv_fd_path.c_str();
    }
};
...
void ASTExporter<ATDWriter>::dumpSourceLocation(SourceLocation Loc) {
  ...
  std::string filePath = Options.normalizeSourceFilePath(Context, PLoc.getFileID());
  if (filePath.empty())
  {
      filePath = Options.normalizeSourcePath(PLoc.getFilename());
  }

  if (filePath != LastLocFilename) {
...
void ASTExporter<ATDWriter>::VisitTranslationUnitDecl(
  ...
  OF.emitString(Options.inputPath);
...
void ASTExporter<ATDWriter>::VisitVarDecl(const VarDecl *D) {
  ...
  bool IsInitICE = D->hasConstantInitialization();
...
void ASTExporter<ATDWriter>::VisitObjCMethodDecl(const ObjCMethodDecl *D) {
  ...
  Mangler->mangleObjCMethodNameAsSourceName(D, StrOS);
...
void ASTExporter<ATDWriter>::VisitObjCAvailabilityCheckExpr(
  ...
  if (HasVersion) {
    ...
    ObjCAvailabilityCheckExpr *E = const_cast<ObjCAvailabilityCheckExpr *>(Expr);
...
namespace clang
{
	...
	private:
		...
		ASTExporterOutputFormat OutputFmt;
        astexport::ASTExporterOptions OP;

	public:
		ExporterASTConsumer(const CompilerInstance &CI,
			ASTExporterOutputFormat OutPutFormat,
			std::unique_ptr<raw_ostream> &&OS) : CI(CI), OS(std::move(OS)), OutputFmt(OutPutFormat), OP(CI) {}
		virtual void HandleTranslationUnit(ASTContext &Ctx)
		{
            ...
            switch (OutputFmt)
            {
            case clang::AEOF_BINIOU:
            {
                astexport::ASTExporter<astexport::ATDWriter::BiniouWriter<llvm::raw_ostream> > P(*OS, Ctx, OP);
                P.dumpDecl(D);
                break;
            }
            case clang::AEOF_YOJSON:
            {
                OP.atdWriterOptions.useYojson = true;
                astexport::ASTExporter<astexport::JsonWriter> P(*OS, Ctx, OP);
                P.dumpDecl(D);
                break;
            }
            case clang::AEOF_JSON:
            {
				astexport::ASTExporter<astexport::JsonWriter> P(*OS, Ctx, OP);
				P.dumpDecl(D);
				break;
