...
class IfCondition : public PostStmt {
public:
  IfCondition(const IfStmt *S, const LocationContext *L,
                const ProgramPointTag *tag = nullptr)
    : PostStmt(S, IfConditionKind, L, tag) {}

private:
  friend class ProgramPoint;
  IfCondition() = default;
  static bool isKind(const ProgramPoint &Location) {
    return Location.getKind() == IfConditionKind;
  }
};
