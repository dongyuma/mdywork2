...
public:
  ...
  const llvm::APFloat& getFloatValue(float floatNumber);
  ...
  const llvm::APFloat &getTruthFloatValue(bool b) {
      if (b)
          return getFloatValue(1.0); //llvm::APFloat(1.0);
      else
          return getFloatValue(0.0); //llvm::APFloat(0.0);
  }
